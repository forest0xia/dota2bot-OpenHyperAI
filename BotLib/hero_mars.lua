...
function X.ConsiderSpearOfMars()
	...
	then
		...
		if #nEnemyLaneCreeps >= 4 and nLocationAoECount >= 4 then
	...
	if J.IsFarming(bot) and nManaAfter > 0.35 and not J.IsLateGame() then
		...
		then
			...
			if ((#nEnemyCreeps >= 4 and nLocationAoECount >= 4) or (#nEnemyCreeps >= 2 and nLocationAoECount >= 2 and nEnemyCreeps[1]:IsAncientCreep()))
			then
				return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
			end
		end
...
function X.ConsiderArenaOfBlood()
	...
	then
		...
		and not J.IsLocationInArena(nLocationAoETarget, nRadius)
		then
			return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
		end
	end
        ...
        do
			...
			then
				...
				then
					...
					local nTargetLocInRangeEnemy = J.GetEnemiesNearLoc(nLocationAoETarget, nRadius)
