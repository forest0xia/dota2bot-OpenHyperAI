...
function X.ConsiderSwashbuckle()
    ...
    then
        ...
        and bot:IsFacingLocation(nLocationAoETarget, 10)
        then
            return BOT_ACTION_DESIRE_HIGH, bot:GetLocation()
        end
	...
	then
        ...
        then
            if GetUnitToLocationDistance(bot, nLocationAoETarget) <= nDashRange
            then
                return BOT_ACTION_DESIRE_HIGH, bot:GetLocation()
            end

            if GetUnitToLocationDistance(bot, nLocationAoETarget) <= nDashRange + nSwashRange
            and not (GetUnitToLocationDistance(bot, nLocationAoETarget) <= nDashRange)
            then
                return BOT_ACTION_DESIRE_HIGH, J.Site.GetXUnitsTowardsLocation(bot, nLocationAoETarget, nDashRange)
            end
        end
	end
