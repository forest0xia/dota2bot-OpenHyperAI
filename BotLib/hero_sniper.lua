...
function X.ConsiderQ()
	...
	local nCanHurtCreepCount = nCanHurtCreepsLocationAoECount
	if nCanHurtCreepsLocationAoE == nil
		or J.GetInLocLaneCreepCount( bot, 1600, nRadius, nCanHurtCreepsLocationAoETarget ) <= 2	--检查半径内是否真的有小兵
	then
...
local nCanHurtHeroCount = 0
if nCanHurtHeroLocationAoE ~= nil then
	nCanHurtHeroCount = nCanHurtHeroLocationAoECount
end

	--对多个敌方英雄使用
	if #nEnemysHeroesInSkillRange >= 2
		and nCanHurtHeroCount >= 2
		...
		and nCanHurtHeroLocationAoE ~= nil
		and not X.IsAbiltyQCastedHere( nCanHurtHeroLocationAoETarget, nRadius )
	then
	...
	then
		...
		if nCanHurtHeroLocationAoENearbyCount >= 2
			and not X.IsAbiltyQCastedHere( nCanHurtHeroLocationAoENearbyTarget, nRadius )
		then
	...
	if #hEnemyHeroList == 0
		...
		and ( nLV < 20 or GetUnitToLocationDistance( bot, nCanHurtCreepsLocationAoETarget ) >= 1100 )
		and not X.IsAbiltyQCastedHere( nCanHurtCreepsLocationAoETarget, nRadius )
	then
	...
	then
		...
		then
			...
			if nAoECount >= 5
			then
				return BOT_ACTION_DESIRE_HIGH, nAoETarget
			end
		end
	end
	...
	then
		...
		then
			...
			local nFarmAoECount = 0
			local nFarmAoETarget = nil
			if nFarmAoE ~= nil then
			    nFarmAoECount = nFarmAoE.count
			    nFarmAoETarget = nFarmAoE.targetloc
			end
			if nFarmAoECount >= 3
				and not X.IsAbiltyQCastedHere( nFarmAoETarget, nRadius )
			then
				return BOT_ACTION_DESIRE_HIGH, nFarmAoETarget
			end
		end
	end
