...
function X.ConsiderOnslaught()
    ...
    if J.IsFarming(bot) or (J.IsPushing(bot) and #nInRangeAlly <= 1) or (J.IsDefending(bot) and #nEnemyHeroes == 0) then
        ...
        then
            ...
            if ((#nCreeps >= 4 and nLocationAoECount >= 4) and not J.HasItem(bot, 'item_radiance'))
            or (#nCreeps >= 2 and nLocationAoECount >= 2 and nCreeps[1]:IsAncientCreep())
            then
                local dist = GetUnitToLocationDistance(bot, nLocationAoETarget)
                OnslaughtETA = RemapValClamped(dist, 100, nDistance, 0.3, nChannelTime)
                bot.onslaught_status = {'farm', nLocationAoETarget}
