...
function X.ConsiderChakram()
	...
	then
		...
		and nMana > 0.33
		then
			local e = (GetUnitToLocationDistance(bot, nLocationAoETarget) / nSpeed) + nCastPoint
			return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget, e
		end
	end
...
function X.ConsiderTwistedChakram()
	...
	then
		...
		and not J.IsRunning(tCreeps[1]) then
			...
			if (nLocationAoECount >= 3 or nLocationAoECount >= 1 and tCreeps[1]:IsAncientCreep())
			then
				return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
			end
		end
