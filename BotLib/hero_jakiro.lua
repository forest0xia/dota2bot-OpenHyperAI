...
function X.ConsiderQ()
	...
	then
		...
		if #nInRangeEnemy >= 2
		then
			return BOT_ACTION_DESIRE_LOW, nLocationAoETarget
		end
	end
	...
	then
		...
		if nLocationAoECount >= 2 and #nEnemyHeroes >= 2
		then
...
function X.ConsiderW()
	...
	then
		if #nEnemyHeroes > 0 and bot:WasRecentlyDamagedByAnyHero( 2.0 ) then
			...
			if locationAoECount >= 1 and #hNearEnemyHeroList >= 1
			then
				return BOT_ACTION_DESIRE_LOW, locationAoETarget
			end
		end
	end
	...
	then
		...
		if ( locationAoECount >= 6 and #lanecreeps >= 6 )
		then
			return BOT_ACTION_DESIRE_LOW, locationAoETarget
		end
	end
	...
	then
		...
		local nInRangeEnemy = J.GetEnemiesNearLoc(locationAoETarget, nRadius)
		if #nInRangeEnemy >= 2
		then
			return BOT_ACTION_DESIRE_LOW, locationAoETarget
		end
