...
function X.ConsiderStarBreaker()
	...
	then
        ...
        and not IsTargetLocInBigUlt(nLocationAoETarget)
        then
    ...
    then
        ...
        if nNeutralCreeps ~= nil and ((#nNeutralCreeps >= 3 and nLocationAoECount >= 3)
                                    or (#nNeutralCreeps >= 2 and nNeutralCreeps[1]:IsAncientCreep() and nLocationAoECount >= 3))
        then
            return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
        end
    end
    ...
    then
        ...
        then
            ...
            then
                ...
                local aLocationAoECount = 0
                local aLocationAoETarget = nil
                if aLocationAoE ~= nil then
                    aLocationAoECount = aLocationAoE.count
                    aLocationAoETarget = aLocationAoE.targetloc
                end
                ...
                local eLocationAoECount = 0
                local eLocationAoETarget = nil
                if eLocationAoE ~= nil then
                    eLocationAoECount = eLocationAoE.count
                    eLocationAoETarget = eLocationAoE.targetloc
                end
                local nInRangeEnemy = J.GetNearbyHeroes(bot,nRadius, true, BOT_MODE_NONE)

                if aLocationAoECount >= 1 and eLocationAoECount >= 1
                then
                    if nInRangeEnemy ~= nil and #nInRangeEnemy >= 1
                    ...
                    return BOT_ACTION_DESIRE_HIGH, aLocationAoETarget
                end
            end
        end
    end
