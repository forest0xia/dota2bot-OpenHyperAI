...
function X.ConsiderWildAxes()
	...
	then
        ...
        if nLocationAoE ~= nil then
            nLocationAoECount = nLocationAoECount
            nLocationAoETarget = nLocationAoETarget
        end
...
function X.ConsiderCallOfTheWildHawk()
    ...
    then
        ...
        if nLocationAoECount >= 2
        then
            local realEnemyCount = J.GetEnemiesNearLoc(nLocationAoETarget, 500)
            if realEnemyCount ~= nil and #realEnemyCount >= 2
            then
                return BOT_ACTION_DESIRE_HIGH
            end
