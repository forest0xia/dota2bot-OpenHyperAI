...
function X.ConsiderBreatheFire()
	...
	if J.IsInTeamFight(bot, 1200) then
		...
		if nLocationAoECount >= 2 and fManaAfter > fManaThreshold2 then
	...
	if J.IsFarming(bot) and bAttacking and fManaAfter > fManaThreshold2 then
		for _, creep in pairs(nEnemyCreeps) do
            if J.IsValid(creep) and J.CanBeAttacked(creep) and not J.IsRunning(creep) then
                ...
                if (nLocationAoECount >= 3 and not bInDragonForm)
				or (nLocationAoECount >= 2 and creep:IsAncientCreep())
				or (nLocationAoECount >= 2 and fManaAfter > fManaThreshold1)
				then
                    return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
                end
            end
