		...
		for _, creep in pairs(nEnemyCreeps) do
			...
			then
                ...
                if string.find(sCreepName, 'ranged') then
                    ...
                    if nLocationAoECount > 0 or J.IsUnitTargetedByTower(creep, false) then
    ...
    if J.IsDefending(bot) and bAttacking and fManaAfter > fManaThreshold1 then
        for _, creep in pairs(nEnemyCreeps) do
            if J.IsValid(creep) and J.CanBeAttacked(creep) and not J.IsRunning(creep) then
                ...
                if (nLocationAoECount >= 4)
                or (nLocationAoECount >= 3 and string.find(creep:GetUnitName(), 'upgraded'))
                then
                    return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
                end
            end
    ...
    if J.IsFarming(bot) and bAttacking and fManaAfter > fManaThreshold1 then
        for _, creep in pairs(nEnemyCreeps) do
            if J.IsValid(creep) and J.CanBeAttacked(creep) and not J.IsRunning(creep) then
                ...
                if (nLocationAoECount >= 3)
                or (nLocationAoECount >= 2 and creep:IsAncientCreep())
                or (nLocationAoECount >= 1 and creep:GetHealth() >= 550)
                then
                    return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
                end
            end
