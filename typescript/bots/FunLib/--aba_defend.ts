...
import { GameStates, IsPingedByAnyPlayer, IsValidHero, HighGroundTowers } from "bots/FunLib/utils";
...
export function GetDefendDesireHelper(bot: Unit, lane: Lane): BotModeDesire {
    let defendDesire = 0;
    const enemiesInRange = jmz.GetLastSeenEnemiesNearLoc(bot.GetLocation(), 2200);
    ...
    if (enemiesInRange.length > 0 && GetUnitToLocationDistance(bot, GetLaneFrontLocation(team, lane, 0)) < 1000) {
    ...
    if (
        (jmz.IsDoingRoshan(bot) && jmz.GetAlliesNearLoc(bot.GetLocation(), 2800).length >= 3) ||
        (jmz.IsDoingTormentor(bot) && jmz.GetAlliesNearLoc(jmz.GetTormentorLocation(team), 900).length >= 2 && jmz.GetEnemiesAroundAncient(bot, 2200) > 0)
    ) {
        return BotModeDesire.None;
    }
    ...
    if (GameTime() - GameStates.defendPings.pingedTime > PING_TIME_DELTA) {
        ...
        for (const towerId of HighGroundTowers) {
            ...
            if (tower !== null && tower.GetHealth() / tower.GetMaxHealth() < 0.8 && jmz.GetLastSeenEnemiesNearLoc(tower.GetLocation(), 1200).length >= 1) {
                defendLocation = tower.GetLocation();
                enemyIsPushingBase = true;
            }
        }
        if (!enemyIsPushingBase && jmz.GetLastSeenEnemiesNearLoc(GetAncient(team).GetLocation(), 1200).length >= 1) {
            defendLocation = GetAncient(team).GetLocation();
            enemyIsPushingBase = true;
        }

        if (defendLocation !== null && enemyIsPushingBase) {
            const saferLocation = add(jmz.AdjustLocationWithOffsetTowardsFountain(defendLocation, 850), RandomVector(50));
    ...
    const laneTowersDestroyed = GetTower(team, Tower.Top3) === null && GetTower(team, Tower.Bot3) === null;
    if (enemiesAroundAncient >= 1 && (midTowerDestroyed || laneTowersDestroyed) && lane === Lane.Mid) {
    ...
    return RemapValClamped(jmz.GetHP(bot), 1, 0, Clamp(defendDesire, 0, 1.25), BotActionDesire.None);
}
...
export function DefendThink(bot: Unit, lane: Lane) {
    ...
    const saferLocation = jmz.AdjustLocationWithOffsetTowardsFountain(laneFrontLocation, 260);
    ...
    if (distanceToLane > 3500 && !bot.WasRecentlyDamagedByAnyHero(2)) {
        ...
        bot.Action_UseAbilityOnLocation(teleports, add(bestTeleportLocation, RandomVector(30)));
        return;
    }

    if (distanceToLane > 2000 && distanceToLane <= 3000 && !bot.WasRecentlyDamagedByAnyHero(3)) {
        bot.Action_MoveToLocation(add(saferLocation, RandomVector(30)));
    } else if (distanceToLane <= 2000 && bot.GetTarget() === null) {
        const nearbyHeroes = jmz.GetHeroesNearLocation(true, laneFrontLocation, 1300);
    ...
    if (distanceToLane < ENEMY_SEARCH_RANGE) {
        ...
        const enemySearchRange = (attackRange < 600 && 600) || math.min(attackRange + 100, ENEMY_SEARCH_RANGE);
        const enemiesInRange = bot.GetNearbyHeroes(enemySearchRange, true, BotMode.None);
        // Changed from
        ...
        if (nearbyLaneCreeps.length > 0) {
            let targetCreep = nearbyLaneCreeps.reduce((prev: Unit | null, current) => {
                const attack = prev ? prev.GetAttackDamage() : 0;
                if (jmz.IsValid(current) && jmz.CanBeAttacked(current) && current.GetAttackDamage() > attack) {
                    return current;
                }
                return prev;
            }, null);
...
export function GetFurthestBuildingOnLane(lane: Lane): LuaMultiReturn<[Unit, number] | [null, number]> {
    const bot = GetBot();
    ...
    for (const towerConfig of laneBuilding.towers) {
        ...
        return $multi(tower, RemapValClamped(health, 0.25, 1, towerConfig.mulMin, towerConfig.mulMax));
    }
...
export function GetEnemyAmountMul(lane: Lane) {
    ...
    return RemapValClamped(heroCount, 1, 3, 1, 2) * RemapValClamped(creepCount, 1, 5, 1, 1.25) * urgency;
}

export function GetEnemyCountInLane(lane: Lane, isHero: boolean): number {
    const laneFront = GetLaneFrontLocation(GetTeam(), lane, 0);
    const units = isHero ? GetUnitList(UnitType.EnemyHeroes) : GetUnitList(UnitType.EnemyCreeps);
