...
function X.ConsiderMagneticField()
	...
	then
		...
		if ( locationAoECount >= 2 ) then
			local targetAllies = J.GetAlliesNearLoc( locationAoETarget, nRadius )
			...
			then
				return BOT_ACTION_DESIRE_HIGH, locationAoETarget
			end
		end
	end
...
function X.ConsiderSparkWraith()
	...
	then
		...
		if locationAoECount >= 1
			and not bot:HasModifier( "modifier_silencer_curse_of_the_silent" )
		then
			local nCreep = J.GetVulnerableUnitNearLoc( bot, false, true, 1600, nRadius, locationAoETarget )
			if nCreep == nil
